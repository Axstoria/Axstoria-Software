...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		// When true, built-in click-to-select raycasting is skipped.
		// Use AddTarget()/ClearTargets() externally to drive selection instead.
		public bool useExternalSelection = false;
		...
		// Fired after each drag completes. Subscribe to sync the domain model.
		public Action onTransformCompleted;

		public UnityEngine.Camera myCamera {get; private set;}
		...
		// Called by built-in render pipeline
		void OnPostRender()
		{
			DrawGizmo();
		}
		...
		void DrawGizmo()
		{
			...
			// Explicit matrix setup required for URP (and harmless for built-in)
			GL.PushMatrix();
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			if(onTransformCompleted != null) onTransformCompleted();
		}
		...
		void GetTarget()
		{
			if(useExternalSelection) return;
		...
		void SetMaterial()
		{
			...
			{
				lineMaterial    = new Material(Resources.Load<Shader>("Lines"));
				outlineMaterial = new Material(Resources.Load<Shader>("Outline"));
			}
		}
	}
}
